...
def build_and_run(translated: str,
                  src="main.cpp", exe="main", std="c++17"):
        ...
        if isinstance(node, ast.AST):
            ...
            if hasattr(node, 'inferred_type'):
                if allsimple and len(args) <= 3:
                    return '%s%s(%s)' % (node.__class__.__name__, node.inferred_type, ', '.join(args)), not args
                return '%s%s(%s%s)' % (node.__class__.__name__, node.inferred_type, prefix, sep.join(args)), False
            else:
                if allsimple and len(args) <= 3:
